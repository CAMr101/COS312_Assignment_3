...
public class GridSearchExecutor {
                ...
                while (iterator.hasNext()) {
                    ...
                    if (future.isDone()) {
                        try {
                            ...
                            System.err.println("\nError in trial " + (completedCount + 1) + ": " + e.getCause().getMessage());
    ...
    private void updateSubmissionProgress(int submitted, int total, long startTime) {
        ...
        System.out.printf(Locale.US, "\rSubmission Progress: %d/%d (%.2f%%) | Speed: %.2f tasks/min | Remaining: %s min (%s hrs)",
    ...
    private void updateProcessingProgress(int completed, int total, long startTime) {
        ...
        System.out.printf(Locale.US, "\rProcessing Progress: %d/%d (%.2f%%) | Speed: %.2f trials/min | Remaining: %s min (%s hrs)",
                completed, total, percentCompleted,
                processingSpeed,
                remainingMinutesStr, remainingHoursStr);

        // Ensure the line ends with a newline if all tasks are completed
        if (completed == total) {
            System.out.println();
        }
    }
    ...
    private void writeBufferedResults(List<GridSearchResult> buffer, FileWriter writer) throws IOException {
        ...
        writer.flush();
        System.out.println("\n[INFO] Wrote " + buffer.size() + " results to CSV.");
        // Re-print the processing progress line after writing results
        // This ensures the progress display is restored after the INFO message
        // It's important to capture current `completedCount`, `totalTrials`, and `processingStartTime`
        // from the `executeFullGridSearch` method to make this call
        // For a clean implementation, we'll pass these as parameters to a helper for re-display
        // but for now, we'll rely on the next `updateProcessingProgress` call in the loop
        buffer.clear();
    }
